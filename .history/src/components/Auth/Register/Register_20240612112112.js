...
const Register = () => {
  ...
  const handleRegister = async(registerUsername, registerFullName, registerPassword, registerGender, registerLatitude, registerLongitude, registeravatarLink, registerEmail, registerLastLoginIP) => {
    ...
    const res = await callLogin(registerUsername, registerFullName, registerPassword, registerGender, registerLatitude, registerLongitude, registeravatarLink, registerEmail, registerLastLoginIP);
    if(res?.data) {
      console.log("res",res)
      // localStorage.setItem('jwt',res.data.jwt)
      // dispatch(doLoginAction(res.config.data))
