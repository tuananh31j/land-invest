...
const Register = () => {
  ...
  Username, FullName, Gender, Latitude, Longitude, avatarLink, Email, LastLoginIP
  ...
  const handleRegister = async(Username, FullName, Gender, Latitude, Longitude, avatarLink, Email, LastLoginIP) => {
    const res = await callLogin(Username, FullName, Gender, Latitude, Longitude, avatarLink, Email, LastLoginIP);
    if(res?.data) {
      console.log("res",res)
      // localStorage.setItem('jwt',res.data.jwt)
      // dispatch(doLoginAction(res.config.data))
