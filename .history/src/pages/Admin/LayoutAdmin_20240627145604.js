...
import { doListGroup, dogetIdBox } from "../../redux/listGroup/listGroupSlice ";
...
const LayoutAdmin = () => {
  ...
  const idGroup = useSelector((state) => state.listgroup.idGroup);
  const idPost = useSelector((state) => state.listgroup.idPost);
  ...
  const handleClick = (boxID) => {
    ...
    dispatch(dogetIdBox(boxID));
  };

  // const handleClickPost = (boxID) => {
  //   setListGroupId(boxID);
  //   dispatch(doListGroup(boxID));
  // };
  ...
  // const listPostName = listPosts.map((post) => ({
  //   label: (
  //     <Link
  //       to="/admin/listpost"
  //       onClick={() => handleClick(box.BoxID)}
